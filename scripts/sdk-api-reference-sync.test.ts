const root = new URL("../", import.meta.url);

interface ReferenceExpectation {
  path: string;
  required: string[];
}

const references: ReferenceExpectation[] = [
  {
    path: "docs/en/sdk/rust/api/server.md",
    required: [
      "Result<NnrpServerOperation, RuntimeError>",
      "`NnrpServerOperation` Replies",
      "send_result_drop",
      "send_progress",
      "send_partial_result",
      "Operation-scoped reply methods are not exposed on",
    ],
  },
  {
    path: "docs/zh/sdk/rust/api/server.md",
    required: [
      "Result<NnrpServerOperation, RuntimeError>",
      "`NnrpServerOperation` 回复",
      "send_result_drop",
      "send_progress",
      "send_partial_result",
      "不暴露任何绕过 operation ownership 的回复方法",
    ],
  },
  {
    path: "docs/en/sdk/rust/api/client.md",
    required: [
      "## `NnrpResult`",
      "`operation_id`",
      "`terminal_state`",
      "`NnrpTerminalEvent`",
      "## `OperationLifecycleEvent`",
      "`Result<NnrpClientRoleEvent, RuntimeError>`",
      "NnrpClientRoleEvent::Lifecycle",
      "`state`",
    ],
  },
  {
    path: "docs/zh/sdk/rust/api/client.md",
    required: [
      "## `NnrpResult`",
      "`operation_id`",
      "`terminal_state`",
      "`NnrpTerminalEvent`",
      "## `OperationLifecycleEvent`",
      "`Result<NnrpClientRoleEvent, RuntimeError>`",
      "NnrpClientRoleEvent::Lifecycle",
      "`state`",
    ],
  },
  {
    path: "docs/en/sdk/python/api/client.md",
    required: [
      "`NativeRuntimeResult`",
      "`operation_id`",
      "`terminal_state`",
      "`NativeTerminalEvent`",
      "### `OperationLifecycleEvent`",
      "`state`",
      "`NativeClientOptions`",
      "`NativeClientSessionOptions`",
      "`NativeSessionRecoveryTicket`",
      "`next_event(timeout=None)`",
      "`NativeClientEvent`",
      "await connection.open_session",
      "await connection.resume_session",
    ],
  },
  {
    path: "docs/zh/sdk/python/api/client.md",
    required: [
      "`NativeRuntimeResult`",
      "`operation_id`",
      "`terminal_state`",
      "`NativeTerminalEvent`",
      "### `OperationLifecycleEvent`",
      "`state`",
      "`NativeClientOptions`",
      "`NativeClientSessionOptions`",
      "`NativeSessionRecoveryTicket`",
      "`next_event(timeout=None)`",
      "`NativeClientEvent`",
      "await connection.open_session",
      "await connection.resume_session",
    ],
  },
  {
    path: "docs/en/sdk/python/api/server.md",
    required: [
      "`NativeServerBootstrapOptions`",
      "`NativeServerSessionOptions`",
      "`NativeServerAcceptOptions` containing only `timeout_ms`",
      "`NativeServerSessionPolicyDecision`",
      "NativeServerEvent | None",
      "async def evaluate",
      "async def next_event",
      "async def receive_submit",
      "async def send_result",
      "send_result_drop",
      "send_progress",
      "send_partial_result",
      "has no parallel operation-reply methods",
      "## Packet Transport Diagnostics",
    ],
  },
  {
    path: "docs/zh/sdk/python/api/server.md",
    required: [
      "`NativeServerBootstrapOptions`",
      "`NativeServerSessionOptions`",
      "只包含 `timeout_ms` 的 `NativeServerAcceptOptions`",
      "`NativeServerSessionPolicyDecision`",
      "NativeServerEvent | None",
      "async def evaluate",
      "async def next_event",
      "async def receive_submit",
      "async def send_result",
      "send_result_drop",
      "send_progress",
      "send_partial_result",
      "不提供任何并行的 operation 回复方法",
      "## 数据包传输诊断接口",
    ],
  },
  {
    path: "docs/en/sdk/javascript/api/core.md",
    required: [
      "interface NnrpResult",
      "operationId: bigint",
      "terminalState: NnrpResultTerminalState",
      "event: NnrpTerminalEvent",
      "type NnrpTerminalEvent",
      "type NnrpClientEvent",
      "interface NnrpOperationLifecycleEvent",
      "state: NnrpOperationState",
    ],
  },
  {
    path: "docs/zh/sdk/javascript/api/core.md",
    required: [
      "interface NnrpResult",
      "operationId: bigint",
      "terminalState: NnrpResultTerminalState",
      "event: NnrpTerminalEvent",
      "type NnrpTerminalEvent",
      "type NnrpClientEvent",
      "interface NnrpOperationLifecycleEvent",
      "state: NnrpOperationState",
    ],
  },
  {
    path: "docs/en/sdk/javascript/api/client.md",
    required: ["Promise<NnrpClientEvent>", "AsyncIterable<NnrpClientEvent>"],
  },
  {
    path: "docs/zh/sdk/javascript/api/client.md",
    required: ["Promise<NnrpClientEvent>", "AsyncIterable<NnrpClientEvent>"],
  },
  {
    path: "docs/en/sdk/javascript/api/server.md",
    required: [
      "Promise<NnrpServerEvent>",
      "`submit`",
      "`runtime`",
      "`lifecycle`",
      "NnrpServerSession.nextEvent",
      "NnrpServerSession.receiveSubmit",
      "NnrpServerOperation",
      "sendResultDrop",
      "sendProgress",
      "sendPartialResult",
      "does not expose parallel operation-reply",
    ],
  },
  {
    path: "docs/zh/sdk/javascript/api/server.md",
    required: [
      "Promise<NnrpServerEvent>",
      "`submit`",
      "`runtime`",
      "`lifecycle`",
      "NnrpServerSession.nextEvent",
      "NnrpServerSession.receiveSubmit",
      "NnrpServerOperation",
      "sendResultDrop",
      "sendProgress",
      "sendPartialResult",
      "不暴露任何并行的 operation 回复方法",
    ],
  },
  {
    path: "docs/en/sdk/csharp/api/client.md",
    required: [
      "### `NnrpResult`",
      "`OperationId`",
      "`TerminalState`",
      "`NnrpTerminalEvent`",
      "`NnrpTerminalEvent.Kind`",
      "ValueTask<NnrpClientEvent>",
      "`NnrpClientEvent.Kind`",
      "`Match<TResult>(Func<NnrpRuntimeEvent, TResult>, Func<NnrpOperationLifecycleEvent, TResult>)`",
      "### `NnrpOperationLifecycleEvent`",
      "`State`",
      "`RequestedSessionId`",
      "`DefaultDeadlineMilliseconds`",
      "`MaxInFlightOperations`",
      "`LeaseTtlHintMilliseconds`",
      "`AllowResume`",
      "`ResumeTokenBytes`",
      "`CacheHints`",
      "`NnrpSessionRecoveryTicket?`",
      "`NnrpSessionRecoveryTicket.FromBytes(ReadOnlySpan<byte>)`",
    ],
  },
  {
    path: "docs/zh/sdk/csharp/api/client.md",
    required: [
      "### `NnrpResult`",
      "`OperationId`",
      "`TerminalState`",
      "`NnrpTerminalEvent`",
      "`NnrpTerminalEvent.Kind`",
      "ValueTask<NnrpClientEvent>",
      "`NnrpClientEvent.Kind`",
      "`Match<TResult>(Func<NnrpRuntimeEvent, TResult>, Func<NnrpOperationLifecycleEvent, TResult>)`",
      "### `NnrpOperationLifecycleEvent`",
      "`State`",
      "`RequestedSessionId`",
      "`DefaultDeadlineMilliseconds`",
      "`MaxInFlightOperations`",
      "`LeaseTtlHintMilliseconds`",
      "`AllowResume`",
      "`ResumeTokenBytes`",
      "`CacheHints`",
      "`NnrpSessionRecoveryTicket?`",
      "`NnrpSessionRecoveryTicket.FromBytes(ReadOnlySpan<byte>)`",
    ],
  },
  {
    path: "docs/en/sdk/csharp/api/server.md",
    required: [
      "`NnrpServerSessionOptions`",
      "`SupportedProfiles`",
      "`SupportedCacheObjects`",
      "`MaxCacheObjects`",
      "`MaxCacheObjectBytes`",
      "`SchemaRegistry`",
      "`ResumeTokenBytes`",
      "`MaxInFlightOperations`",
      "`GrantedOperationCredit`",
      "`LeaseTtlMilliseconds`",
      "`ResumeWindowMilliseconds`",
      "`ApplicationPolicy`",
      "`INnrpServerSessionPolicy`",
      "ValueTask<NnrpServerEvent>",
      "`NnrpServerEvent.Kind`",
      "ValueTask<NnrpServerSessionPolicyDecision> EvaluateAsync(SessionOpenMetadata open);",
      "contains only `TimeoutMilliseconds`",
      "SendResultAsync",
      "SendResultDropAsync",
      "SendProgressAsync",
      "SendPartialResultAsync",
      "does not expose parallel operation-reply methods",
    ],
  },
  {
    path: "docs/zh/sdk/csharp/api/server.md",
    required: [
      "`NnrpServerSessionOptions`",
      "`SupportedProfiles`",
      "`SupportedCacheObjects`",
      "`MaxCacheObjects`",
      "`MaxCacheObjectBytes`",
      "`SchemaRegistry`",
      "`ResumeTokenBytes`",
      "`MaxInFlightOperations`",
      "`GrantedOperationCredit`",
      "`LeaseTtlMilliseconds`",
      "`ResumeWindowMilliseconds`",
      "`ApplicationPolicy`",
      "`INnrpServerSessionPolicy`",
      "ValueTask<NnrpServerEvent>",
      "`NnrpServerEvent.Kind`",
      "ValueTask<NnrpServerSessionPolicyDecision> EvaluateAsync(SessionOpenMetadata open);",
      "只包含默认值为 `0` 的 `TimeoutMilliseconds`",
      "SendResultAsync",
      "SendResultDropAsync",
      "SendProgressAsync",
      "SendPartialResultAsync",
      "session 不暴露并行的",
      "operation 回复方法",
    ],
  },
  {
    path: "docs/en/sdk/csharp/quick-start.md",
    required: ["application options remain transport-neutral"],
  },
  {
    path: "docs/zh/sdk/csharp/quick-start.md",
    required: ["应用 options 保持 transport-neutral"],
  },
  {
    path: "docs/en/sdk/csharp/api/runtime.md",
    required: [
      "public properties are exactly `Header`, `Metadata`, and `Tail`",
      "`NnrpRuntimeEventTail`",
      "`Match<TResult>(...)`",
      "`MetadataBodyAndDelta`",
    ],
  },
  {
    path: "docs/zh/sdk/csharp/api/runtime.md",
    required: [
      "公开属性严格为",
      "`Header`、`Metadata` 和 `Tail`",
      "`NnrpRuntimeEventTail`",
      "`Match<TResult>(...)`",
      "`MetadataBodyAndDelta`",
    ],
  },
  {
    path: "docs/en/sdk/rust/quick-start.md",
    required: ["result.operation_id", "NnrpTerminalEvent::Runtime", "NnrpTerminalEvent::Lifecycle"],
  },
  {
    path: "docs/zh/sdk/rust/quick-start.md",
    required: ["result.operation_id", "NnrpTerminalEvent::Runtime", "NnrpTerminalEvent::Lifecycle"],
  },
  {
    path: "docs/en/sdk/python/quick-start.md",
    required: [
      "await connection.open_session",
      "result.event.as_runtime()",
      "result.event.as_lifecycle().state",
    ],
  },
  {
    path: "docs/zh/sdk/python/quick-start.md",
    required: [
      "await connection.open_session",
      "result.event.as_runtime()",
      "result.event.as_lifecycle().state",
    ],
  },
];

function assertIncludes(source: string, fragment: string, path: string): void {
  if (!source.includes(fragment)) {
    throw new Error(`${path} is missing frozen SDK API fragment: ${fragment}`);
  }
}

Deno.test("language SDK references publish the frozen result and lifecycle projections", async () => {
  for (const reference of references) {
    const source = await Deno.readTextFile(new URL(reference.path, root));
    for (const fragment of reference.required) {
      assertIncludes(source, fragment, reference.path);
    }
  }
});

Deno.test("language SDK references reject superseded result and lifecycle narratives", async () => {
  const sources = await Promise.all(
    references.map(async (reference) => ({
      path: reference.path,
      source: await Deno.readTextFile(new URL(reference.path, root)),
    })),
  );
  const forbidden = [
    "NnrpClientEvent::ResultDropReason",
    "| `frame_id` | `u32` | Result frame id.",
    "| `frame_id` | `u32` | Result frame id。",
    "`pending`, `dispatched`, `completed`, `dropped`, or `cancelled`",
    "`pending`、`dispatched`、`completed`、`dropped` 或 `cancelled`",
    "For `RESULT_PUSH`, `NativeRuntimeResult` exposes:",
    "对于 `RESULT_PUSH`，`NativeRuntimeResult` 提供：",
    "result.frame_id",
    "readonly event: NnrpRuntimeEvent;",
    "owned terminal runtime event",
    "拥有所有权的终态 runtime event",
    "non-success results preserve the terminal protocol event",
    "非成功结果\n保留建立终态的 protocol event",
    "result.event.header.frame_id",
    "print(result.body)",
    "| Event family | Semantic tail property |",
    "| 事件类别 | 语义化 tail 属性 |",
    ".submit(FrameSubmitMetadata::default(),",
    "NativeClientSessionOpenOptions",
    "NativeClientConnectionOptions",
    "NativeServerOptions",
    "session_handle_id",
    "session_generation",
    "NNRP_NATIVE_BINDING_MODE",
    "cffi_api",
    "`NnrpClientOptions.Transports`",
    "`Transports` | `IReadOnlyList<INnrpNativeTransportProvider>?`",
    "`SessionId`, `SessionGeneration`",
    "`ServerId` | `ulong`",
    "`ServerGeneration`",
    "`Result<NnrpRuntimeEvent, RuntimeError>`",
    "`Promise<NnrpRuntimeEvent>` | Reads the next event",
    "`ValueTask<NnrpRuntimeEvent>` | Returns the next event",
    "`ValueTask<NnrpRuntimeEvent>` | 按当前 session",
  ];

  for (const { path, source } of sources) {
    for (const fragment of forbidden) {
      if (source.includes(fragment)) {
        throw new Error(`${path} still contains superseded SDK API fragment: ${fragment}`);
      }
    }
  }
});
