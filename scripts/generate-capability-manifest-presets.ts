...
async function createConformanceSource(): Promise<ConformanceSource> {
  if (configuredConformanceRoot) {
    const localProtocolRoot = join(configuredConformanceRoot, "protocol");
    if (!(await pathExists(localProtocolRoot))) {
      throw new Error(`NNRP_CONFORMANCE_REPO is set to ${configuredConformanceRoot}, but ${localProtocolRoot} does not exist.`);
    }

    return {
      description: configuredConformanceRoot,
      ...
      readJson: async <T>(path: string) => {
        const absolutePath = join(configuredConformanceRoot, ...splitRepoPath(path));
...
async function main(): Promise<void> {
  ...
  const presets = await collectVersionPresets(source);
  ...
  console.log(`Generated ${presets.length} capability preset versions from ${source.description} into ${outputFile}`);
}
