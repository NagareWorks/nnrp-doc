...
当前对外只维护这一套公开口径。侧边栏入口旁边标注“预览”，只是说明稳定版编号尚未发布；阅读和接入时，直接把这一页当作当前 `NNRP/1` 的版本入口即可。

如果只先记住几件事，优先记这四点：

1. `tensor` 和 `token` 是并列的标准 profile，公共层不再为某一种业务场景定制字段。
2. 连接、session、operation 三层拆开：一次实时交互不是"一发一收"，而是提交流、结果流、控制消息三条通路并行运转。
3. 帧承载边界不写死：协议支持握手前探测路径质量，运行中也可以迁移承载路径，不用断连重建。
4. 公共头固定 40B，消息里带自描述长度，在 QUIC、TCP+TLS、IPC、WebSocket 或其他可靠二进制帧承载上都能用同一套解包逻辑。
...
协议不绑死某种承载方式。宿主可以在握手前对多条候选路径（QUIC、TCP+TLS、IPC、WebSocket 等）做质量探测，然后选出更合适的一条正式建连。运行中如果网络质量变化，也可以通过 `SESSION_MIGRATE` 在不重建 session 的前提下切换承载路径。
