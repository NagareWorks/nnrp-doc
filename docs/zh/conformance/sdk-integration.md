...
仓库只负责实现自己的 adapter 入口，以及按需附带证据文件。

| 接入面 | 职责 | suite 与实现仓库交换什么 | |---|---|---|---| | capability selection |
决定当前实现会被选择哪些公共 case | capability manifest、conformance report | | canonical vectors |
从可读 recipe 生成确定性的字节级 fixture | suite 生成的 vector manifest | | adapter execution |
执行选中的动态行为用例并返回机器可读结果 | execution plan、adapter command、case-result report | |
benchmark execution | 测量选中的延迟与吞吐场景 | benchmark plan、benchmark command、benchmark result
report | | API profile execution | 验证 adapter 级 OpenAI-compatible API 表面 | API profile
capability manifest、profile execution plan、API profile results |
...
suite action 不直接调用你的测试框架，而是调用你提供的 adapter command。这个命令必须消费 suite 生成的
...
::: tip 这里不要再写嵌入式 conformance 测试 正式接入边界是“suite-owned plan + adapter command +
...
提供 OpenAI-compatible NNRP API 表面的 adapter 需要单独维护一份 API profile capability
...
Adapter command 应消费这份 plan，对自己的 OpenAI-compatible NNRP 表面逐条执行 recipe，并写出 API
