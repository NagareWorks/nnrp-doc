...
2. 通过 TCP 或 QUIC adapter 获得连接。
3. 调用 [`connect_client_session`](#connect-client-session) 或
   [`connect_client_session_with_probe`](#connect-client-session-with-probe)。
4. 简单请求用 [`ClientSession.submit`](#clientsession-submit)；多帧并发用
   [`send_submit`](#clientsession-send-submit) + [`receive_result`](#clientsession-receive-result)。
5. 结束时调用 [`ClientSession.close`](#clientsession-close)。

## `connect_client_session`

完成控制面握手并返回可用的 [`ClientSession`](#clientsession)。

| 参数 | 类型 | 必填 | 取值 / 范围 | 说明 |
|---|---|---:|---|---|
| `connection` | `NnrpQuicConnection \| NnrpTcpConnection` | 是 | 已连接 transport | 来自 [Transport Adapters](./transport)。 |
| `profile` | [`ClientProfile`](#clientprofile) | 是 | 配置对象 | 客户端 capability 和 cache 偏好。 |
| `dial_policy` | [`ClientDialPolicy`](#clientdialpolicy) | 是 | transport 策略 | 写入 `CLIENT_HELLO` 的 transport 信息。 |
| `auth_block` | `bytes` | 否 | 默认 `b""` | 应用自定义认证负载。 |

| 返回 | 可能抛出 |
|---|---|
| [`ClientSession`](#clientsession) | transport、握手解析、capability 拒绝错误。 |
...
连接前执行 transport 探测，并把选中的 transport 写入握手。

| 参数 | 类型 | 必填 | 取值 / 范围 | 说明 |
|---|---|---:|---|---|
| `connection` | `NnrpQuicConnection \| NnrpTcpConnection` | 是 | 已连接 transport | 当前实际连接。 |
| `profile` | [`ClientProfile`](#clientprofile) | 是 | 配置对象 | capability 和 cache 偏好。 |
| `auth_block` | `bytes` | 否 | 默认 `b""` | 应用认证负载。 |

| 返回 | 可能抛出 |
|---|---|
| [`ClientSession`](#clientsession) | probe、transport 或握手失败。 |
