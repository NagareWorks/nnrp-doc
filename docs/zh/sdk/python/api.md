...
Preview4 wheel 使用 transport-scoped native artifact。`load_native_runtime(..., transport="tcp")`、`load_native_client(..., transport="ipc")` 这类入口会校验 artifact manifest、ABI `1.11.x`、协议版本和 transport slot。生产路径应优先使用 `nnrp.client.connect_native_client_connection(require_native=True, transport=...)`。
