...
export const wireConformancePresets = [
  {
    ...
    "note": {
      "zh": "从线路级测试场景声明派生，覆盖 22 个由测试套件直接扮演客户端、服务端或代理的协议级场景。",
      "en": "Derived from wire-level conformance scenario manifests. Covers 22 protocol scenarios where the runner directly acts as client, server, or proxy."
    },
    ...
    "scenarios": [
      ...
      {
        "id": "wire.profile.openai-compatible.level1.ipc",
        "mode": "suite_as_client",
        "status": "mandatory",
        "feature": "profile.openai-compatible.level1.wire",
        "requiredCapabilities": [
        ...
        "description": "Runner sends the frozen OpenAI-compatible typed request directly over IPC, then validates raw partial-result JSON and a typed terminal result over the live provider endpoint.",
        "summary": {
          "zh": "线路级测试场景会直接交换 NNRP 帧，并校验终态、关键帧和观测证据。",
          "en": "Runner sends the frozen OpenAI-compatible typed request directly over IPC, then validates raw partial-result JSON and a typed terminal result over the live provider endpoint."
        },
        "steps": [
          {
            "action": "send",
      ...
      {
        "id": "wire.profile.openai-compatible.level1.websocket",
        "mode": "suite_as_client",
        "status": "mandatory",
        "feature": "profile.openai-compatible.level1.wire",
        "requiredCapabilities": [
        ...
        "description": "Runner sends the frozen OpenAI-compatible typed request directly over WSS, then validates raw partial-result JSON and a typed terminal result over the live provider endpoint.",
        "summary": {
          "zh": "线路级测试场景会直接交换 NNRP 帧，并校验终态、关键帧和观测证据。",
          "en": "Runner sends the frozen OpenAI-compatible typed request directly over WSS, then validates raw partial-result JSON and a typed terminal result over the live provider endpoint."
        },
        "steps": [
          {
            "action": "send",
